...
typedef struct _CustomData
{
  ...
  GstElement *sendAudioPipeline;         /* The running pipeline */
  GstElement *recvAudioPipeline;         /* The running pipeline */
...
gchar *sendAudioPipeline;
gchar *recvAudioPipeline;
...
state_changed_cb (GstBus * bus, GstMessage * msg, CustomData * data)
{
  ...
  if (GST_MESSAGE_SRC (msg) == GST_OBJECT (data->sendAudioPipeline)) {
...
app_function (void *userdata)
{
  ...
  GST_DEBUG ("Audio pipeline Send : %s \nRecv : %s", sendAudioPipeline, recvAudioPipeline);
  data->sendAudioPipeline = gst_parse_launch (sendAudioPipeline, &error);
  ...
  bus = gst_element_get_bus (data->sendVideoPipeline);
...
gst_native_init (JNIEnv * env, jobject thiz,
  ...
  if (sendAudio != NULL && (*env)->GetStringUTFLength (env, sendAudio) > 0)
    sendAudioPipeline = (gchar*)(*env)->GetStringUTFChars (env, sendAudio, NULL);
  if (recvAudio != NULL && (*env)->GetStringUTFLength (env, recvAudio) > 0)
    recvAudioPipeline = (gchar*)(*env)->GetStringUTFChars (env, recvAudio, NULL);
...
gst_native_pause (JNIEnv * env, jobject thiz)
{
  ...
  if (data->sendAudioPipeline != NULL) gst_element_set_state (data->sendAudioPipeline, GST_STATE_PAUSED);
  if (data->recvAudioPipeline != NULL) gst_element_set_state (data->recvAudioPipeline, GST_STATE_PAUSED);
}
